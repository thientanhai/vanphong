...
class ResidoController extends PublicController
{
    ...
    public function ajaxGetCitiesfull(Request $request, CityInterface $cityRepository, BaseHttpResponse $response)
    {
        if (!$request->ajax()) {
            abort(404);
        }
        ...
        $cities = $cityRepository->filters($keyword, -1, [], ['cities.*']);
